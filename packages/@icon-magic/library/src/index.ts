import { ProcessStatus, minify, subscribe } from '@icon-magic/imagemin-farm';
...
import { decode, encode } from './png-metadata';
...
export function readIcon(dir: string): Icon {
  const manifest: Icon = JSON.parse(
...
async function processIcon(
  ...
  const webpOut = await convertToWebp(out);
...
export async function iconToPNGs(icon: string | Icon) {
  ...
  for (const asset of icon.assets) {
    const svg = await fs.readFile(path.join(icon.path, asset.path), {
      encoding: 'utf8'
    });
    const hash = crypto
      .createHash('md5')
    ...
    for (const res of asset.resolutions) {
      for (const size of asset.sizes) {
        for (const w of Object.values(getWidth(size))) {
          for (const h of Object.values(getHeight(size))) {
            const realSize = `${w * res}x${h * res}`;
            const out = path.join(tmpDir, `${icon.name}_${realSize}.png`);
